    ...
    private Table toTableDefinition(SQLCreateTableStatement sqlStatement) {
        ...
        sqlStatement.getTableElementList()
                ...
                .forEach(col -> {
                    ...
                    if (SQLArrayDataType.class.isAssignableFrom(col.getDataType().getClass())) {
                        SQLArrayDataType arrayDataType = (SQLArrayDataType) col.getDataType();
                        SQLDataType componentType = arrayDataType.getComponentType();
                        Class<?> componentJavaType = SqlTypeMapper.getType(componentType.getName());
                        column.setType(Array.newInstance(componentJavaType, 0).getClass());
                    } else {
